...
@section('main')
    ...
    <div class="cbox-fluid col-md-3" id="specData">
        ...
        <div class="bcontent">
            <dl>
                <dt>Naam:</dt> <dd>{{$specialism->name}}</dd>
                <dt>Beschrijving:</dt> <dd>{{$specialism->description}}</dd>
                <dt>Korte Beschrijving:</dt> <dd>{{$specialism->short_description}}</dd>

            </dl>
            @if( auth()->user()->hasAccess('admin.specialisms.edit'))
                <div class="text-center">
                    <form class="col-md-12" method="GET" action="{{route('admin.specialisms.edit', $specialism->id)}}">
                        <div class="form-group text-center">
                            <input type="submit" class="form-control btn btn-edit" value="Bewerken"/>
                        </div>
                    </form>
                    @if(Auth::user()->hasAccess('admin.specialisms.destroy'))
                        <form class="col-md-12" method="POST" action="{{route('admin.specialisms.destroy', $specialism->id)}}" onsubmit="return confirm('Weet je zeker dat je {{$specialism->name}} wilt verwijderen?\nDEZE BEWERKING KAN NIET TERUGGEDRAAID WORDEN')">
                            {{csrf_field()}}
                            {{method_field('delete')}}
                            <div class="form-group text-center">

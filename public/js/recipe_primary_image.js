...
/////// Cropper Options set
    ...
    var options = {
        ...
        crop: function(e) {
            $("#primary_image_cropped").val(parseInt(e.detail.width) + "," + parseInt(e.detail.height) + "," + parseInt(e.detail.x) + "," + parseInt(e.detail.y) + "," + parseInt(e.detail.rotate));
        }
    };
...
///// Saving Image with Ajax Call
    $("body").on("click", "#Save", function() {
        var form_data = $('#newrecipePrimary')[0];
        $.ajax({
            url: "/admin/recepten/nieuw/primaryimage", // Url to which the request is send
            type: "POST",
            ...
            {
                $('#primary_image_filename').val(filename);
...
////// When user upload image
    $(document).on("change", "#primary_image", function() {

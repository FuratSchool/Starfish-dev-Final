...
/////// Cropper Options set
    ...
    var options = {
        ...
        crop: function(e) {
            $("#profile_image_cropped").val(parseInt(e.detail.width) + "," + parseInt(e.detail.height) + "," + parseInt(e.detail.x) + "," + parseInt(e.detail.y) + "," + parseInt(e.detail.rotate));
        }
    };
...
///// Saving Image with Ajax Call
    $("body").on("click", "#Save", function() {
        var form_data = $('#newspec')[0];
        $.ajax({
            url: "/admin/specialisten/nieuw/profielafbeelding", // Url to which the request is send
            type: "POST",
            ...
            {
                $('#profile_image_filename').val(filename);
